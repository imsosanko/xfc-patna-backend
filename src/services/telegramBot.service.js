...
`;
    ...
    const keyboard = {
      inline_keyboard: [
        [{ text: '🌟 Join Xiaomi Circle', url: XIAOMI_CIRCLE_URL }],
        [{ text: '📢 Join XFC Patna Group', url: XFC_GROUP_URL }],
        [{ text: '🚀 Launch XFC Patna App', web_app: { url: MINI_APP_URL } }],
...
`;
    ...
    const keyboard = {
      inline_keyboard: [
        [{ text: '🌟 Join Xiaomi Circle', url: XIAOMI_CIRCLE_URL }],
        [{ text: '📢 Join XFC Group', url: XFC_GROUP_URL }],
      ],
    };
...
const setWebhook = async (webhookUrl) => {
  ...
  try {
    ...
    console.error('❌ Webhook set failed:', response.data?.description);
    return false;
  } catch (error) {
    const errorMsg =
      error.response?.data?.description || error.message || 'Unknown error';
    console.error('❌ Error setting webhook:', errorMsg);
    return false;
  }
};
